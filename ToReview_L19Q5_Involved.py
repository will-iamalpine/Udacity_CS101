...
def involved2(courses, person):
    involved = {}
    for semester in courses:
        in_class = []
        for x in courses[semester]:
            x_dict = courses[semester][x]
            if x_dict.get('teacher') == person:
                in_class.append(x)
        if len(in_class) > 0:
            involved[semester] = in_class
    return involved
    
#my solution
def involved(courses, person):
    involved = {}
    for semester in courses: #print semester
        in_class =[]
        for classes in courses[semester]: #print classes
            for fields in courses[semester][classes]: #print fields
                if person == courses[semester][classes][fields]:
                    in_class.append(classes) #print semester,classes, 'in_class', in_class
        if in_class != []:
            involved.update({semester:in_class}) #print courses[hexamester][classes][fields], person, 'found'
    return involved

#udacity solution
def involved_udacity(courses, person):
    involved = {}
    for semester in courses: #print semester
        for classes in courses[semester]: #print classes
            for fields in courses[semester][classes]: #print fields
                if person == courses[semester][classes][fields]:
                    if semester in involved:
                        involved[semester].append(classes)
                    else:
                        involved[semester] = [classes]
    return involved

# For example:

print involved_udacity(courses, 'Dave')
